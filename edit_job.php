...
$locations = [
    'Antipolo & Rizal' => [
        ...
        'items' => [
            ...
            'MR D.I.Y. Ayala Malls Feliz, Pasig (near Antipolo)',
        ],
    ],
    'Marikina' => [
        'icon'  => '🏙️',
...
// Pre-select values from DB (or repopulated POST on error)
$sel_title    = $row['title']    ?? '';
$sel_location = $row['location'] ?? '';
$sel_status   = $row['status']   ?? 'Open';

// Helper: find group data for a selected value
function findGroupData($val, $data) {
    foreach ($data as $group => $gd) {
        if (in_array($val, $gd['items'])) return $gd;
    }
    return null;
}
?>
...
<head>
    ...
    <title>Edit Job — MR D.I.Y Admin</title>
    ...
    .salary-input { padding-left: 54px !important; }
...
<div class="main">
    ...
    <div class="page-header">
        <div>
            ...
            <div class="page-subtitle">Update the details for this listing</div>
        </div>
    ...
    <div class="form-card">
        <form method="POST" id="jobForm">
            ...
            <!-- Hidden real inputs (submitted to PHP) -->
                ...
                <div class="cs-wrap" id="wrapTitle">
                    <div class="cs-trigger field-surface" id="trigTitle" onclick="toggleDrop('title')">
                        <div class="cs-display">
                            <span id="dispTitle">
                                <?php $tg = findGroupData($sel_title, $job_titles); ?>
                                <?php if ($sel_title && $tg): ?>
                                    <span class="cs-badge" style="background:<?= $tg['color'] ?>18;color:<?= $tg['color'] ?>"><?= $tg['icon'] ?> <?= htmlspecialchars($sel_title) ?></span>
                                <?php else: ?>
            ...
            <div class="field-block">
                ...
                <textarea name="description" class="field-surface"
                          placeholder="Describe the role, responsibilities, and requirements…"><?= htmlspecialchars($row['description']) ?></textarea>
            </div>
                        ...
                        <div class="cs-trigger field-surface" id="trigLocation" onclick="toggleDrop('location')">
                            <div class="cs-display">
                                <span id="dispLocation">
                                    <?php $lg = findGroupData($sel_location, $locations); ?>
                                    <?php if ($sel_location && $lg): ?>
                                        <span class="cs-badge" style="background:<?= $lg['color'] ?>18;color:<?= $lg['color'] ?>"><?= $lg['icon'] ?> <?= htmlspecialchars($sel_location) ?></span>
                                    <?php else: ?>
            ...
            <div class="form-actions">
                <button type="submit" name="update" class="btn btn-primary btn-lg" onclick="return validateForm()">
...
// Init with DB value on page load
pickStatus('<?= addslashes($sel_status) ?>');
